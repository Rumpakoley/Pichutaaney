  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-black/70 backdrop-blur-sm flex items-center justify-center p-3 sm:p-6 font-sans">
      <div className="bg-[#ECE5DA] border border-[#D5CBBD] rounded-3xl shadow-2xl w-full max-w-5xl max-h-[90vh] flex flex-col overflow-hidden text-left">
        ...
        <div className="px-6 py-4 border-b border-[#D5CBBD] bg-[#ECE5DA] flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
          ...
          <div className="flex items-center space-x-2">
            <div className="relative flex-1 sm:w-60">
              ...
              <input
                ...
                className="w-full pl-9 pr-4 py-2 text-xs rounded-full bg-[#F7F3EC] border border-[#D5CBBD] focus:outline-none focus:border-[#28221D] text-[#28221D] placeholder:text-[#8C867D]"
              />
            </div>
            ...
            {activeTab === 'waitlist' && waitlist.length > 0 && (
              <button
                ...
                className="px-4 py-2 rounded-full text-[10px] uppercase tracking-widest font-semibold bg-[#F7F3EC] border border-[#D5CBBD] hover:border-[#28221D] text-[#28221D] flex items-center space-x-1.5 cursor-pointer shadow-xs"
                title="Export to CSV"
              >
            ...
            <div className="space-y-3">
              {filteredWaitlist.length === 0 ? (
                <div className="p-10 rounded-2xl text-center text-[#655B51] bg-[#F7F3EC] border border-[#D5CBBD]">
                  ...
                  <p className="font-marcellus text-xl text-[#28221D]">No waitlist entries found.</p>
                  ...
                  <table className="w-full text-left">
                    ...
                    <tbody className="divide-y divide-[#D5CBBD]">
                      {filteredWaitlist.map((item) => (
                        <tr key={item.id} className="hover:bg-[#ECE5DA]/60">
                          <td className="p-3.5 font-mono font-semibold text-[#28221D]">{item.id}</td>
                          <td className="p-3.5 font-medium text-[#28221D]">{item.fullName}</td>
                          <td className="p-3.5">
                            <div className="text-[#28221D]">{item.email}</div>
                            {item.phone && <div className="text-[#8C867D] text-[10px]">{item.phone}</div>}
                          </td>
                          <td className="p-3.5 text-[#655B51]">{item.city}</td>
                          <td className="p-3.5 font-semibold text-[#28221D]">{item.partySize}</td>
                          <td className="p-3.5 text-[#655B51] max-w-[200px] truncate" title={item.dietaryPreferences.join(', ')}>
                          ...
                          <td className="p-3.5 text-right">
                            <select
                              ...
                              className="text-[11px] bg-white border border-[#D5CBBD] rounded-full px-2 py-1 text-[#28221D]"
                            >
            ...
            <div className="space-y-3">
              {filteredInquiries.length === 0 ? (
                <div className="p-10 rounded-2xl text-center text-[#655B51] bg-[#F7F3EC] border border-[#D5CBBD]">
                  ...
                  <p className="font-marcellus text-xl text-[#28221D]">No private event inquiries yet.</p>
                  <p className="text-xs mt-1 font-sans font-light">When someone submits an event inquiry, it will show up here.</p>
                </div>
              ) : (
                <div className="space-y-3">
                  {filteredInquiries.map((inq) => (
                    <div key={inq.id} className="p-5 bg-[#F7F3EC] rounded-2xl border border-[#D5CBBD] space-y-3 shadow-xs">
