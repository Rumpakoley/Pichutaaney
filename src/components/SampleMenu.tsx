  ...
  return (
    <section id="sample-menu" className="py-20 sm:py-28 bg-[#FAFAF9] border-b border-[#E4E4E7] overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <StaggerContainer staggerDelay={0.1} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 text-left">
          {SAMPLE_SEASONAL_MENU.map((course, idx) => {
            const isHighlight = idx === 0;
            return (
              <StaggerItem key={course.courseNumber} className="h-full">
                <div
                  className={`border p-6 h-full flex flex-col justify-between transition-all duration-300 hover:shadow-lg hover:-translate-y-1 ${
                    isHighlight 
        ...
        <RevealText delay={0.2} yOffset={25}>
          <div className="mt-12 bg-white p-6 border border-[#E4E4E7] flex flex-col sm:flex-row items-center justify-between gap-4 text-left shadow-2xs hover:border-[#853724]/40 transition-colors">
